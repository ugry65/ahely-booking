...
- kötelező e-mail issue #107 / draft PR #111: minden create/update/delete után a booking owner kap levelet, admin műveletnél is; sorozatnál 1 összefoglaló e-mail; elkészült a verziózott outbox, lease/retry/dead-letter RPC, a booking műveletek tranzakcióvégi enqueue-integrációja, a magyar text/HTML renderer, a verzióra rögzített Nodemailer-kliens, a Bearer tokennel védett Node.js worker Route Handler, az append-only heartbeat audit és az admin-only kézbesítési monitor; helyben 23 tesztfájl / 134 teszt, typecheck és build PASS, a monitor commitján Application checks #583, Database tests #532 (52 fájl / 749 pgTAP, minden konkurenciateszt és schema lint) és Vercel PASS; a runtime alapértéke `disabled`, staging DB-alkalmazás, scheduler, secret, capture UAT és valós SMTP UAT még nyitott;
