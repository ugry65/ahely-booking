...
A #111 draft ágon elkészült az outbox, a booking RPC-k tranzakcióvégi enqueue-integrációja, a magyar renderer, a verzióra rögzített Nodemailer-kliens, a Bearer tokennel védett Node.js worker Route Handler, az append-only worker heartbeat audit és az admin-only, érzékeny adatot nem visszaadó kézbesítési monitor. A runtime alapértéke `BOOKING_EMAIL_MODE=disabled`, amely nem claimel, nem küld és nem ír heartbeatot. A 2026-09-04-i read-only staging audit szerint a 135 legacy outbox rekord megvan, az új booking-email migrációk még nincsenek alkalmazva, a Vault aktív, a `pg_cron` és `pg_net` nincs engedélyezve. Az SMTP nélküli capture bevezetési kapuit a `docs/BOOKING_EMAIL_STAGING_CAPTURE_RUNBOOK.md` rögzíti. Scheduler, runtime secret, staging DB-alkalmazás, capture UAT és valós SMTP UAT még nyitott; main merge és production deploy nem történt. A monitor checkpoint commitján (`1660e61`) Application checks #583, Database tests #532 (52 fájl / 749 pgTAP, minden konkurenciateszt és schema lint) és Vercel PASS.
...
- kötelező e-mail issue #107 / draft PR #111: minden create/update/delete után a booking owner kap levelet, admin műveletnél is; sorozatnál 1 összefoglaló e-mail; elkészült a verziózott outbox, lease/retry/dead-letter RPC, a booking műveletek tranzakcióvégi enqueue-integrációja, a magyar text/HTML renderer, a verzióra rögzített Nodemailer-kliens, a Bearer tokennel védett Node.js worker Route Handler, az append-only heartbeat audit, az admin-only kézbesítési monitor és a staging capture runbook; helyben 23 tesztfájl / 134 teszt, typecheck és build PASS, a monitor commitján Application checks #583, Database tests #532 (52 fájl / 749 pgTAP, minden konkurenciateszt és schema lint) és Vercel PASS; a runtime alapértéke `disabled`, staging DB-alkalmazás, scheduler, secret, capture UAT és valós SMTP UAT még nyitott;
