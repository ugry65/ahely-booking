...
require_env STAGING_DRILL_ARTIFACT
require_env STAGING_DRILL_AGE_IDENTITY
require_env BACKUP_GDRIVE_REMOTE
require_env BACKUP_B2_REMOTE

if [[ "$STAGING_DRILL_ARTIFACT" =~ / ]] ||
  [[ ! "$STAGING_DRILL_ARTIFACT" =~ ^ahely-booking-staging-drill_[0-9]{8}T[0-9]{6}Z_[0-9a-f]{12}\.tar\.gz\.age$ ]]; then
  echo "Invalid staging drill artifact name" >&2
  exit 1
fi
...
download_and_verify() {
  ...
  local artifact_path="$target_dir/$STAGING_DRILL_ARTIFACT"
  local sidecar_path="$artifact_path.sha256"

  rclone copyto "$remote_base/$STAGING_DRILL_ARTIFACT" "$artifact_path" --no-traverse
  rclone copyto "$remote_base/$STAGING_DRILL_ARTIFACT.sha256" "$sidecar_path" --no-traverse
  (
    cd "$target_dir"
...
cmp "$gdrive_dir/$STAGING_DRILL_ARTIFACT" "$b2_dir/$STAGING_DRILL_ARTIFACT"
cmp "$gdrive_dir/$STAGING_DRILL_ARTIFACT.sha256" "$b2_dir/$STAGING_DRILL_ARTIFACT.sha256"

identity_file="$work_dir/staging-drill-age-identity.txt"
printf '%s\n' "$STAGING_DRILL_AGE_IDENTITY" > "$identity_file"
plain_bundle="$work_dir/restore.tar.gz"
age --decrypt \
  --identity "$identity_file" \
  --output "$plain_bundle" \
  "$gdrive_dir/$STAGING_DRILL_ARTIFACT"
...
printf 'STAGING DRILL isolated restore PASS: %s\n' "$STAGING_DRILL_ARTIFACT"
