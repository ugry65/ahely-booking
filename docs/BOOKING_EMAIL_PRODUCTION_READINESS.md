> **Recurring real-provider UAT – PASS (2026-09-25):** a #150 sorozatösszefoglaló valós staging SMTP/Resend útvonalon, kontrollált saját címzettre sikeresen kézbesült. Heti 4 tervezett alkalom / 1 kivétel mellett az immutable snapshot 3 érintett foglalást, első és utolsó alkalmat, `Hetente` ismétlődést és a 2026-10-26 kimaradt alkalmat tartalmazta. Outbox: `sent`, 1 attempt, provider message id, worker HTTP 200; a címzett emberileg is PASS-nak minősítette a tartalmat és megjelenést. Bizonyíték: `docs/evidence/2026-09-25-staging/RECURRING_BOOKING_EMAIL_PROVIDER_UAT.md`. A tesztsorozat auditálható módon lemondásra került. Staginget a teszt után vissza kell állítani nem-valós-küldési módba; production aktiválás továbbra is külön release-gate.\n\n> **#150 lezárási frissítés – 2026-09-25:** PR #197 mainbe került, merge SHA: `2e5c9a5035670b469897471ca4d768a9a35e7044`. A merge előtti Application checks, Database tests és Release evidence mind PASS. A Database tests első futása azért bukott, mert az új bridge-definíció véletlenül elhagyta a korábban kötelező, adatmentes SQLSTATE warningot; ezt a merge előtt visszaállítottuk, majd a teljes DB tesztkészlet és a külön bridge-warning integrációs teszt is PASS lett. A staging Supabase-re a `booking_email_recurring_summary` migráció 2026-09-25-én sikeresen felkerült (staging history verzió: `20260925095053`), a magyar recurrence mapping DB-ből ellenőrizve: Naponta / Hetente / Kéthetente / Havonta. A staging-web main deployment `dpl_8BFw1EU18gg7wmyUGpq1AQZMCdf2` READY és pontosan a merge SHA-t futtatja. A sorozatösszefoglaló első alkalom, utolsó alkalom, ismétlődés és kimaradt alkalmak mezőit immutable outbox snapshotból rendereli; kivétel hiányában `Nincs`. **A még hiányzó release-gate a célzott valós provider recurring UAT; production e-mail aktiválás továbbra sincs engedélyezve.**
