...
-- Business decision 2026-09-24: AllBooked booking notes may contain operationally
-- important information. Preserve them on the booking instead of dropping them.
-- Public calendar reads still do not expose notes; management reads expose them only
-- to the booking owner or an active admin.
...
begin
  ...
  if exists (
    ...
    where jsonb_typeof(item) <> 'object'
       or not item ?& array['sourceFingerprint','roomName','startLocal','endLocal','durationMinutes','bookingTitle','note','useType']
       or item - array['sourceFingerprint','roomName','startLocal','endLocal','durationMinutes','bookingTitle','note','useType'] <> '{}'::jsonb
       ...
       or (item -> 'note' <> 'null'::jsonb and jsonb_typeof(item -> 'note') <> 'string')
  ...
  for v_item in select item from jsonb_array_elements(p_bookings) item order by item ->> 'startLocal', item ->> 'roomName' loop
    ...
    if found then
      if v_existing.user_id <> p_user_id or v_existing.room_id <> v_room.id or v_existing.start_at <> v_start_at or v_existing.end_at <> v_end_at
         ...
         or v_existing.note is distinct from nullif(v_item ->> 'note', '') or v_existing.booking_title is distinct from nullif(v_item ->> 'bookingTitle', '') then
      ...
      values (v_room.id,p_user_id,p_actor_id,v_start_at,v_end_at,(v_item ->> 'useType')::public.booking_use_type,'active',nullif(v_item ->> 'note',''),nullif(v_item ->> 'bookingTitle',''),gen_random_uuid())
...
comment on function public.admin_import_allbooked_customer(uuid,uuid,text,text,text,text,text[],jsonb,uuid) is
  'Fail-closed, idempotent and atomic one-customer AllBooked import. Service role only; preserves booking notes and excludes legacy financial data.';
