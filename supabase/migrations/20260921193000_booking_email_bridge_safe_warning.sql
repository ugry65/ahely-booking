...
-- Already applied bridge migration 202609030002 is immutable. Replace only the
-- function body; existing trigger and grants are preserved. Never log SQLERRM.
...
exception
  when others then
    -- SQLSTATE is a fixed five-character error class, never a recipient, note,
    -- payload, or raw error detail. PostgreSQL emits this after the failed enqueue
    -- subtransaction has been rolled back, without failing the booking.
