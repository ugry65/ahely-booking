...
create or replace function public.admin_set_other_booker_names_visible(
  p_user_id uuid,
  p_visible boolean,
...
begin
  ...
  if p_visible is null then
    raise exception 'A névláthatósági beállítás kötelező.' using errcode = '22004';
  end if;

  select jsonb_build_object('other_booker_names_visible', profile.other_booker_names_visible)
  into v_before
  ...
  set other_booker_names_visible = p_visible,
      updated_at = now()
  where id = p_user_id
    and other_booker_names_visible is distinct from p_visible;
  ...
  if v_before is distinct from v_after then
    insert into public.audit_logs (
      ...
      v_actor_id, 'profile.booking_name_visibility.updated', 'profile', p_user_id::text,
...
begin
  ...
  where booking.status = 'active'
    ...
    and (
      v_actor.role = 'admin'
      or exists (
        select 1 from public.user_room_permissions direct_permission
        where direct_permission.user_id = v_actor.id
          and direct_permission.room_id = room.id
          and direct_permission.can_book
      )
      or exists (
        select 1
        from public.access_group_members membership
        join public.access_groups access_group
          on access_group.id = membership.group_id and access_group.is_active
        ...
        where membership.user_id = v_actor.id
          and group_permission.room_id = room.id
          and group_permission.can_book
      )
    )
...
revoke execute on function public.admin_set_other_booker_names_visible(uuid,boolean,uuid)
  from public, anon;
revoke execute on function public.list_calendar_bookings(timestamptz,timestamptz)
  from public, anon;
grant execute on function public.admin_set_other_booker_names_visible(uuid,boolean,uuid)
  to authenticated;
...
comment on function public.list_calendar_bookings(timestamptz,timestamptz) is
  'Jogosultságszűrt naptár-read model; más foglaló neve a megtekintő profilbeállítása szerint maszkolt.';
comment on column public.profiles.other_booker_names_visible is
  'A profil tulajdonosa megtekintőként láthatja-e más foglalók nevét az engedélyezett helyiségek naptárában.';
