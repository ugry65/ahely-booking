...
const serviceDate = new Intl.DateTimeFormat("hu-HU", {
...
function skippedDatesLabel(values: string[] | undefined): string {
  if (!values?.length) return "Nincs";
  return values.map((value) => serviceDate.format(new Date(`${value}T12:00:00Z`))).join(", ");
}
...
export function renderBookingEmail(
  ...
  const firstOccurrence = dateTime.format(new Date(payload.firstStartAt));
  const lastOccurrence = dateTime.format(new Date(payload.lastEndAt));
  ...
  const details = [
    ...
    ...(payload.recurrence ? [
      ...
      `Első alkalom: ${firstOccurrence}`,
      `Utolsó alkalom: ${lastOccurrence}`,
      `Ismétlődés: ${payload.recurrence}`,
      `Kimaradt alkalmak: ${skippedDatesLabel(payload.skippedDates)}`,
    ] : summary ? [
      `Érintett alkalmak: ${payload.affectedCount}`,
  ...
  const updateHtml = eventType === "booking.updated" && payload.before && payload.after
    ...
    : "";
  const summaryRows = payload.recurrence
    ? `${row("Érintett alkalmak", String(payload.affectedCount))}${row("Első alkalom", firstOccurrence)}${row("Utolsó alkalom", lastOccurrence)}${row("Ismétlődés", payload.recurrence)}${row("Kimaradt alkalmak", skippedDatesLabel(payload.skippedDates))}`
    : summary
