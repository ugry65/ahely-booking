...
function assertStagingTarget() {
  ...
  if (hostname !== `${STAGING_PROJECT_REF}.supabase.co` || (ref && ref !== "staging")) {
    throw new Error("Az író AllBooked import kizárólag a staging branch staging Supabase projektjén engedélyezett.");
  }
}
...
export async function POST(request: Request) {
  ...
  if (confirmation !== PAPP_DALMA_IMPORT_CONFIRMATION) {
    return NextResponse.json({ error: "A staging import megerősítő szövege nem egyezik." }, { status: 400 });
  }
  ...
  if (userId) {
    ...
    if (error || !data.user) {
      return NextResponse.json({ error: "A staging Auth-felhasználó létrehozása nem sikerült." }, { status: 500 });
    }
    userId = data.user.id;
    authCreated = true;
  }
