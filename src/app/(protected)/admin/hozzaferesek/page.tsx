...
export default async function RoomAccessAdminPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <section className="stack">
    <header className="page-heading"><div><p className="eyebrow">Adminisztráció</p><h1>Közvetlen user-jogok</h1><p className="muted">Átmeneti kompatibilitási felület az egyedi user–helyiség kivételekhez és az ismétlődési joghoz. A helyiségek és helyiségcsoportok kezelése már a külön Helyiségek menüben történik.</p></div><Link className="button secondary" href="/admin/helyisegek">Helyiségek</Link></header>
    ...
    <section className="card wide-card stack"><h2>Felhasználók közvetlen helyiségjogai</h2><p className="muted">Itt kizárólag az egyedi közvetlen kivételek és az ismétlődő foglalási jogosultság kezelendő. A csoportból kapott foglalási jog ettől függetlenül érvényesül.</p><div className="admin-permission-list">{overview.users.map((user) => <details key={user.id}><summary>{user.name} · {user.email}{user.is_active ? "" : " · Inaktív"}</summary><div className="admin-record-list">{overview.rooms.map((room) => { const permission = userPermissions.get(pairKey(user.id, room.id)); return <form action={saveUserRoomPermission} className="permission-matrix-row" key={room.id}><input type="hidden" name="userId" value={user.id} /><input type="hidden" name="roomId" value={room.id} /><input type="hidden" name="canBook" value="false" /><input type="hidden" name="canRepeat" value="false" /><strong>{room.name}{room.is_active ? "" : " · Inaktív"}</strong><label className="inline-check"><input type="checkbox" name="canBook" value="true" defaultChecked={permission?.can_book ?? false} /> Közvetlen foglalási jog</label><label className="inline-check"><input type="checkbox" name="canRepeat" value="true" defaultChecked={permission?.can_repeat ?? false} /> Ismételhet</label><button>Mentés</button></form>; })}</div></details>)}</div></section>
  </section>;
}
