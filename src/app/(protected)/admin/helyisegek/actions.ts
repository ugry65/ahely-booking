...
import { parseGroupForm, parsePair, parseRoomForm } from "@/lib/room-access-form";
import { createClient } from "@/lib/supabase/server";

function resultUrl(kind: "hiba" | "uzenet", message: string) {
  return `/admin/helyisegek?${new URLSearchParams({ [kind]: message }).toString()}`;
}
...
async function finish(error: { code?: string; message?: string } | null, success: string, fallback: string) {
  ...
  revalidatePath("/admin/helyisegek");
...
export async function saveRoom(formData: FormData) {
  ...
  if (parsed.value.roomId) {
    const { data: existing } = await supabase.from("rooms")
      .select("is_training_room")
      .eq("id", parsed.value.roomId)
      .maybeSingle<{ is_training_room: boolean }>();
  ...
  const { error } = await supabase.rpc("admin_upsert_room", {
    ...
    p_display_order: parsed.value.displayOrder,
    p_is_training_room: isTrainingRoom,
    p_is_active: checkboxValue(formData, "isActive"),
    p_correlation_id: crypto.randomUUID(),
  });
  await finish(error, "A helyiség adatai elmentve.", "A helyiség mentése nem sikerült.");
}

export async function saveAccessGroup(formData: FormData) {
  await requireAdmin();
  ...
  await finish(error, "A helyiségcsoport adatai elmentve.", "A helyiségcsoport mentése nem sikerült.");
}
...
export async function saveGroupRoomPermission(formData: FormData) {
  ...
  const { error } = await supabase.rpc("admin_set_group_room_permission", {
    p_group_id: pair.value.firstId,
    p_room_id: pair.value.secondId,
    p_can_book: checkboxValue(formData, "canBook"),
    p_can_repeat: false,
    p_correlation_id: crypto.randomUUID(),
  });
