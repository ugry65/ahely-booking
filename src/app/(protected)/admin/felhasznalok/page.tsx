...
export default async function UsersAdminPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return (
    <section className="stack">
      ...
      <div className="admin-grid"><section className="card stack"><h2>Új felhasználó</h2><form action={inviteUser} className="stack"><label>Vezetéknév<input name="lastName" required /></label><label>Keresztnév<input name="firstName" required /></label><label>E-mail<input name="email" type="email" autoComplete="email" required /></label><label>Alapértelmezett jelszó<input name="defaultPassword" type="password" autoComplete="new-password" minLength={12} required /></label><label>Alapértelmezett jelszó még egyszer<input name="defaultPasswordConfirmation" type="password" autoComplete="new-password" minLength={12} required /></label><button type="submit">Felhasználó létrehozása</button></form><p className="muted">A jelszó csak az első belépéshez használható; belépés után a usernek kötelező megváltoztatnia. A létrehozáskor nem küldünk automatikusan levelet.</p></section>
