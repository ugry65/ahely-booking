...
const IMPORT_CONFIRMATION = "IMPORT-PAPP-DALMA-STAGING";
...
export function MigrationDryRunForm() {
  ...
  async function runImport() {
    ...
    try {
      ...
      const response = await fetch("/api/internal/migration-import", { method: "POST", body: formData });
      const body = await response.json();
      if (!response.ok) {
        setError(body.error ?? "A staging import nem futott le.");
        return;
      }
      setImportResult(body.reconciliation as Record<string, unknown>);
    } catch {
      setError("A staging import technikai hiba miatt nem futott le.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="stack">
      ...
      {result?.valid && result.summary.normalizedUsers === 1 && result.summary.normalizedBookings === 21 ? <section className="card stack">
        <div><p className="eyebrow">Staging író import</p><h2>21 foglalás atomi betöltése</h2></div>
        <p className="muted">Csak a staging Supabase cél engedélyezett. Az import `individual` foglalásokat és a Forrás tér csoportjogot hozza létre; legacy árat és payment státuszt nem vesz át, e-mailt nem küld.</p>
