...
export function QuickBookingDialog({ rooms, repeatableRoomIds, selectedDate, today, bookingUsers = [], currentUserId }: { rooms: BookableRoom[]; repeatableRoomIds: string[]; selectedDate: string; today: string; bookingUsers?: BookingUser[]; currentUserId?: string }) {
  ...
  return (
    <>
      <button className="quick-booking-fab" type="button" aria-label="Új foglalás gyors megadása" title="Új foglalás" onClick={() => setOpen(true)} style={{ position: "fixed", right: "2rem", bottom: "2rem", zIndex: 40, width: "3.5rem", height: "3.5rem", borderRadius: "999px", fontSize: "2rem", lineHeight: 1, padding: 0, boxShadow: "0 .5rem 1.25rem rgb(31 42 36 / 24%)" }}>+</button>
