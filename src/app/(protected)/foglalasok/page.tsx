...
export default async function BookingsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return (
    <section className="booking-page stack">
      ...
      {rooms.length ? <QuickBookingDialog rooms={rooms} repeatableRoomIds={repeatableRoomIds} selectedDate={selectedDate} today={today} bookingUsers={bookingUsers} currentUserId={profile.id} isAdmin={profile.role === "admin"} /> : null}
